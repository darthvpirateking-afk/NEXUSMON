    ...
    companion.mode_manager.update_memory(
        {
            ...
            "whitelist": ["api.example.com", "trusted-service.com"],
        }
    )
...
def example_commit_states():
    ...
    print(
        f"   State: {CommitState.ACTION_READY.value if '[ACTION_READY]' in response else 'other'}"
    )
