...
class IntelligenceLayer:
    ...
    def predict_outcome(self, task_context: TaskContext) -> Dict[str, Any]:
        ...
        similar_tasks = [
            log for log in self.execution_logs if log.task_name == task_context.intent
        ]
        ...
        if similar_tasks:
            ...
            success_rate = sum(1 for log in similar_tasks if log.success) / len(
                similar_tasks
            )
        else:
    ...
    def evolve_weights(self, performance_data: Dict[str, float]):
        ...
        for key, value in performance_data.items():
            if key in self.scoring_weights:
                ...
                self.scoring_weights[key] = max(
                    0.1, min(2.0, self.scoring_weights[key] + adjustment)
                )
    ...
    def generate_patchpack(self) -> Optional[Dict[str, Any]]:
        ...
        if len(failed_tasks) > len(recent_logs) * 0.3:  # >30% failure rate
            patchpack = {
                ...
                "changes": {"routing_rules": "more_conservative"},
                "created_at": time.time(),
            }
...
class CompanionMode:
    ...
    def respond(self, user_input: str) -> str:
        ...
        response = (
            f"I understand you're saying: {user_input}. How can I help you today?"
        )
...
class OperatorMode:
    ...
    def execute_task(
        ...
        task_context = TaskContext(
            task_id=task_id, intent=intent, parameters=parameters
        )
        ...
        if commit_state == CommitState.ACTION_READY:
            ...
            if verify_result:
                response_parts.append(
                    f"VERIFY: {len(verify_result.risks)} risks identified"
                )
            ...
            if patchpack:
                response_parts.append("EVOLVE: Patchpack generated")
...
class ModeManager:
    ...
    def detect_mode(self, user_input: str) -> SystemMode:
        ...
        is_question = (
            any(qw in input_lower for qw in question_words) and "?" in user_input
        )
    ...
    def process_input(
        self, user_input: str, parameters: Optional[Dict[str, Any]] = None
    ) -> str:
